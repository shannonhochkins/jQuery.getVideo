(function($) {
    methods = {
        init: function(options) {
            ...
            if (settings.videoID == 'Error') {
                ...
                if (settings.isYouTube) {
                    ...
                    methods.drawIframe.call(container, settings);
        ...
        drawIframe: function(setting) {
            var apiName = (settings.isYouTube ? 'enablejsapi' : 'api');
            var urlName = (settings.isYouTube ? '//www.youtube.com/embed/' : 'http://player.vimeo.com/video/');
            var plyName = (settings.isYouTube ? 'YouTube' : 'Vimeo');
            // Do not add key variables here, add them to the new YT.Player class.
